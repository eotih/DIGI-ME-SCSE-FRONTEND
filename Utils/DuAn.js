...
async function getData(language) {
  if (language === "VI") {
    const data = await getAllNews(
      WEB_API + "Management/GetByIdCategory?idcat=1"
    );
    ...
    const data = await getAllNews(
      WEB_API + "Management/GetByIdCategoryEN?idcat=1"
    );
    const approvedNews = data.filter((news) => news.IDState === 2);
    const sortByNewDate = approvedNews.sort(function (a, b) {
      a = new Date(a.UpdatedByDate);
...
NCDT.click(async function () {
  if (NCDT.is(":checked")) {
    ...
    checkLanguage("NCDT", 0, "Nghiên cứu đào tạo");
  }
});
...
const renderDataWithPagination = (data, numb) => {
  ...
  const html = dataFilter[0].map(function (response) {
    if (data === "VI") {
      ...
      const { IDField, Title, SlugEN, Details, Image, CreatedByDate } =
        response;
...
const checkLanguage = (FieldName, IdField, textContent) => {
  ...
  if (labels === textContent) {
    ...
    renderDataWithPagination("EN", IdField);
  }
};
...
const getFieldBySlug = async () => {
  ...
  if (
    slugResult === "Nghiên cứu - Đào tạo" ||
    slugResult === "Research - Training"
  ) {
  ...
  if (
    slugResult === "Giới và bình đẳng giới" ||
    slugResult === "Gender - Gender equality"
  ) {
